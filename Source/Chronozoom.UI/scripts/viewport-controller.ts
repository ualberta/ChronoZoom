...
module CZ {
    export module ViewportController {
        ...
        export function ViewportController2(setVisible, getViewport, gesturesSource) {
                ...
                if (gesture.Type == "Zoom") {
                    ...
                    if (gesture.Source == "Touch") { // in case of touch input use the previouslyEstimatedViewport
                        if (previouslyEstimatedViewport)
                            ...
                            initialViewport = new CZ.Viewport.Viewport2d(
                                latestViewport.aspectRatio,
                                latestViewport.width,
                                latestViewport.height,
                                new CZ.Viewport.VisibleRegion2d(latestVisible.centerX, latestVisible.centerY, latestVisible.scale)
                            );
                        }
                    } else { // gesture.Source == "Mouse"
                        initialViewport = new CZ.Viewport.Viewport2d(//zoom is always performed over recent viewport state, not the estimated target viewport. Cloning latest viewport (deep copy)
                            latestViewport.aspectRatio,
                            latestViewport.width,
                            latestViewport.height,
                            new CZ.Viewport.VisibleRegion2d(latestVisible.centerX, latestVisible.centerY, latestVisible.scale)
            ...
            */
            this.coerceVisibleHorizontalBound = function (vp) {
                ...
                if (CZ.Settings.maxPermitedTimeRange) {
                    if (visible.centerX > CZ.Settings.maxPermitedTimeRange.right) //canvas right border in virtual coordinates is greater than permited border
                        visible.centerX = CZ.Settings.maxPermitedTimeRange.right;
                    else if (visible.centerX < CZ.Settings.maxPermitedTimeRange.left) //canvas left border in virtual coordinates is less than permited border
            ...
            */
            this.coerceVisibleVerticalBound = function (vp) {
                ...
                if (CZ.Common.maxPermitedVerticalRange) {
                    if (visible.centerY > CZ.Common.maxPermitedVerticalRange.bottom) //canvas top border in virtual coordinates is greater than permited border
                        visible.centerY = CZ.Common.maxPermitedVerticalRange.bottom;
                    else if (visible.centerY < CZ.Common.maxPermitedVerticalRange.top) //canvas bottom border in virtual coordinates is less than permited border
            ...
            */
            this.coerceVisibleInnerZoom = function (vp, gesture) {
                ...
                if (this.effectiveExplorationZoomConstraint) //explicitly set content observation constraint overrides the constraints of timelines
                    ...
                    for (var i = 0; i < CZ.Settings.deeperZoomConstraints.length; i++) { //determinig the region to apply deeper zoom constraint of timelines
            ...
            self.updateRecentViewport = function () {
                var vp = getViewport();
                var vis = vp.visible;
                self.recentViewport = new CZ.Viewport.Viewport2d(
                vp.aspectRatio,
                vp.width,
                vp.height,
                new CZ.Viewport.VisibleRegion2d(
                    vis.centerX,
                    vis.centerY,
                    vis.scale
                    ));
